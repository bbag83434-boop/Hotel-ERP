...
class GoodsReceiveNote(BaseModel):
    ...
    po_id = Column("poId", String(36), ForeignKey("purchase_orders.id", ondelete="SET NULL"), nullable=True)
    grn_number = Column("grnNumber", String(50), unique=True, nullable=False, index=True)
    supplier_invoice_number = Column("supplierInvoiceNumber", String(100), nullable=True)
    invoice_amount = Column("invoiceAmount", Numeric(14, 4), nullable=True)
    ...
    supplierInvoiceNumber = synonym("supplier_invoice_number")
    invoiceAmount = synonym("invoice_amount")
