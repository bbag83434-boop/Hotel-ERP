...
# Normalize common unit symbols/names to the canonical symbols used above.
# IMPORTANT: "GM" is a common ERP unit symbol for gram, so it must normalize
# to "g". Without this alias, GM -> KG incorrectly reports "No conversion".
ALIASES = {
    'gm': 'g',
    'gram': 'g',
    'grams': 'g',
...
def convert_quantity(
    ...
    units = db.query(Unit).filter(
        ...
        Unit.id.in_([from_unit_id, to_unit_id]),
    ).all()
    ...
    fs = ALIASES.get(_symbol(fu), _symbol(fu))
    ts = ALIASES.get(_symbol(tu), _symbol(tu))
    ...
    if factor is None:
        raise ValueError(
            f'No conversion configured from {fu.symbol} to {tu.symbol}'
        )
