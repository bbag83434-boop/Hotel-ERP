...
class InventoryService {
    ...
    // -------------------------------------------------------------
    static async requestStockTransfer(params) {
        const { companyId, fromWarehouseId, toWarehouseId, items, notes, actorId, ipAddress, userAgent } = params;
        if (fromWarehouseId === toWarehouseId) {
            throw new response_utils_1.AppError('Source and destination warehouses cannot be the same', 400);
        }
        if (!items || items.length === 0) {
            throw new response_utils_1.AppError('At least one item must be included in the transfer request', 400);
        }
        const [fromWh, toWh] = await Promise.all([
            database_1.prisma.warehouse.findFirst({ where: { id: fromWarehouseId, companyId } }),
            database_1.prisma.warehouse.findFirst({ where: { id: toWarehouseId, companyId } })
    ...
    static async dispatchStockTransfer(params) {
        ...
        if (!transfer)
            throw new response_utils_1.AppError('Stock transfer not found', 404);
        if (transfer.status !== 'PENDING') {
            throw new response_utils_1.AppError(`Cannot dispatch transfer with status "${transfer.status}"`, 400);
        }
        return database_1.prisma.$transaction(async (tx) => {
            ...
            for (const item of transfer.items) {
                ...
                if (!currentBalance || currentBalance.quantity.lessThan(qty)) {
                    ...
                    throw new response_utils_1.AppError(`Insufficient stock for "${item.item.name}" in ${transfer.fromWarehouse.name}. Available: ${available}, Required for transfer: ${qty}`, 400);
                }
    ...
    static async receiveAndReconcileTransfer(params) {
        ...
        if (transfer.status === 'COMPLETED' || transfer.status === 'CANCELLED') {
            throw new response_utils_1.AppError(`Cannot receive transfer with status "${transfer.status}"`, 400);
        }
        return database_1.prisma.$transaction(async (tx) => {
            let totalTransitLossValue = new client_1.Prisma.Decimal(0);
            for (const recItem of receivedItems) {
                const accepted = new client_1.Prisma.Decimal(recItem.acceptedQty);
                const transitLoss = new client_1.Prisma.Decimal(recItem.transitLossQty || 0);
                const item = await tx.item.findFirst({ where: { id: recItem.itemId, companyId } });
                if (!item)
                    throw new response_utils_1.AppError(`Item not found: ${recItem.itemId}`, 404);
