...
export class AccountingService {
  ...
  public static async getTrialBalance(companyId: string, params: { branchId?: string; asOfDate?: string }) {
  ...
  public static async getBalanceSheet(companyId: string, params: { branchId?: string; asOfDate?: string }) {
    const asOf = params.asOfDate ? new Date(params.asOfDate) : new Date();
    ...
    for (const acc of accounts) {
      ...
      if (acc.type === 'ASSET') {
        ...
        if (netAsset !== 0) {
          ...
          if (acc.subType === 'FIXED_ASSET') nonCurrentAssets.push(item);
        ...
        if (netLiab !== 0) {
          ...
          if (acc.subType === 'LONG_TERM_LIABILITY') longTermLiabilities.push(item);
  ...
  public static async getTaxBreakupReport(
    companyId: string,
    ...
    const [taxPayableAccount, inputTaxCreditAccount] = await Promise.all([
      prisma.chartOfAccount.findFirst({ where: { companyId, code: '2020' } }),
      prisma.chartOfAccount.findFirst({ where: { companyId, code: '1040' } })
    ]);

    const targetAccountIds = [taxPayableAccount?.id, inputTaxCreditAccount?.id].filter(Boolean) as string[];
  ...
  public static async getBankCashReconciliation(
    companyId: string,
    params: { branchId?: string; startDate?: string; endDate?: string }
  ) {
