...
export class ApprovalService {
  ...
  // ==========================================
  public static async evaluateAndTriggerApproval(
    companyId: string,
    data: {
  ...
  public static async createApprovalRequest(
    ...
    if (res.approvalRequest) return res.approvalRequest;
  ...
  public static async updateApprovalRule(
    ...
    const updated = await prisma.approvalRule.update({
      ...
      data: {
        ...
        minAmount: data.minAmount !== undefined ? new Prisma.Decimal(data.minAmount) : existing.minAmount,
  ...
  public static async deleteApprovalRule(companyId: string, ruleId: string, actorId?: string) {
    const existing = await prisma.approvalRule.findFirst({
      where: { id: ruleId, companyId }
    });
    if (!existing) throw new AppError('Approval rule not found', 404);
  ...
  // ==========================================
  public static async getApprovalSummary(companyId: string, branchId?: string) {
