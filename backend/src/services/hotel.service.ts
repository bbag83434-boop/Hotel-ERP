...
export class HotelService {
  ...
  public static async getFloors(companyId: string, branchId: string) {
  ...
  public static async createFloor(
    companyId: string,
    data: { branchId: string; floorNumber: number; name: string; description?: string },
    actorId?: string,
    ipAddress?: string,
    userAgent?: string
  ) {
    ...
    if (existing) throw new AppError(`Floor number ${data.floorNumber} already exists`, 409);
  ...
  public static async getRoomTypes(companyId: string, branchId: string) {
  ...
  public static async createRoomType(
    ...
    if (existing) throw new AppError(`Room type with code ${data.code} already exists`, 409);
  ...
  public static async getRooms(companyId: string, branchId: string, status?: any) {
  ...
  public static async createRoom(
    ...
    if (existing) throw new AppError(`Room number ${data.roomNumber} already exists in this branch`, 409);
  ...
  public static async getGuests(companyId: string, search?: string) {
  ...
  public static async createGuestProfile(
    companyId: string,
    data: {
      firstName: string;
      lastName: string;
  ...
  public static async getBookings(
    companyId: string,
    params: { branchId?: string; status?: any; startDate?: string; endDate?: string }
  ) {
  ...
  public static async createBooking(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const nightlyRate = new Prisma.Decimal(data.roomRate);
      ...
      const advancePaid = new Prisma.Decimal(data.advancePayment || 0);
      ...
      if (data.roomId) {
        ...
        if (!room) throw new AppError('Specified room not found', 404);
        if (room.status === 'OUT_OF_SERVICE') {
          throw new AppError(`Room ${room.roomNumber} is currently out of service`, 400);
        }
      }
  ...
  // Check-In Guest
  public static async checkInGuest(
    companyId: string,
    bookingId: string,
    data: {
      roomId: string;
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (booking.status === 'CHECKED_IN') throw new AppError('Guest is already checked in', 400);
      if (booking.status === 'CANCELLED') throw new AppError('Cannot check in cancelled booking', 400);
      ...
      if (room.status === 'OCCUPIED') {
        throw new AppError(`Room ${room.roomNumber} is currently OCCUPIED by another guest`, 400);
      }
      if (room.status === 'DIRTY_CLEANING') {
        throw new AppError(`Room ${room.roomNumber} is DIRTY and requires housekeeping inspection before check-in`, 400);
      }
      if (room.status === 'OUT_OF_SERVICE') {
        throw new AppError(`Room ${room.roomNumber} is OUT OF SERVICE for maintenance`, 400);
      }
  ...
  public static async postFolioCharge(
    companyId: string,
    bookingId: string,
    data: {
      transactionType: any;
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (booking.status !== 'CHECKED_IN') {
        throw new AppError(`Cannot post charge to booking with status ${booking.status}`, 400);
      }

      const chargeAmount = new Prisma.Decimal(data.amount);
  ...
  // Room Change / Upgrade
  public static async changeRoom(
    companyId: string,
    bookingId: string,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (!booking || !booking.roomId) throw new AppError('Active checked-in booking not found', 404);

      const newRoom = await tx.room.findFirst({ where: { id: data.newRoomId, companyId } });
      if (!newRoom) throw new AppError('Target room not found', 404);
      if (newRoom.status !== 'AVAILABLE' && newRoom.status !== 'INSPECTED') {
        throw new AppError(`Target room ${newRoom.roomNumber} is not available (Status: ${newRoom.status})`, 400);
      }
  ...
  public static async checkOutGuest(
    companyId: string,
    bookingId: string,
    data: {
      paymentMethod: any;
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const discount = new Prisma.Decimal(data.discountAmount || 0);
      const paymentAmount = new Prisma.Decimal(data.amountPaid);
      ...
      await AccountingService.recordHotelFolioJournal({
        companyId,
        branchId: booking.branchId,
        bookingId: booking.id,
  ...
  public static async runNightAudit(
    companyId: string,
    branchId: string,
    auditDateStr?: string,
    actorId?: string,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      let taxRevenue = new Prisma.Decimal(0);
      ...
      const adr = occupiedRooms > 0 ? roomRevenue.dividedBy(occupiedRooms) : new Prisma.Decimal(0);
      const revpar = totalRooms > 0 ? roomRevenue.dividedBy(totalRooms) : new Prisma.Decimal(0);
      ...
      const nightAudit = await tx.nightAudit.upsert({
        ...
        update: {
          totalRooms,
          occupiedRooms,
          occupancyRate: new Prisma.Decimal(occupancyRate),
          roomRevenue,
          totalRevenue,
          adr,
          revpar,
        ...
        create: {
          ...
          occupancyRate: new Prisma.Decimal(occupancyRate),
          roomRevenue,
          fnbRevenue: new Prisma.Decimal(0),
  ...
  public static async getHousekeepingTasks(companyId: string, branchId: string, status?: any) {
  ...
  public static async createHousekeepingTask(
    companyId: string,
    data: {
      branchId: string;
      roomId: string;
  ...
  public static async updateHousekeepingStatus(
    companyId: string,
    taskId: string,
    status: any,
    remarks?: string,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (status === 'COMPLETED' || status === 'INSPECTED') timestamps.completedAt = new Date();
  ...
  public static async getMaintenanceTickets(companyId: string, branchId: string) {
  ...
  public static async createMaintenanceTicket(
    companyId: string,
    data: {
      branchId: string;
      roomId?: string | null;
