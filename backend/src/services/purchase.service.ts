...
export class PurchaseService {
  ...
  public static async returnGoodsToSupplier(params: {
    companyId: string;
    branchId: string;
    warehouseId: string;
    supplierId: string;
    ...
    if (!items || items.length === 0) {
      throw new AppError('At least one item must be specified for supplier return', 400);
    }
    ...
    return prisma.$transaction(async (tx) => {
      ...
      let totalReturnAmount = new Prisma.Decimal(0);

      for (const line of items) {
        const returnQty = new Prisma.Decimal(line.returnQty);
        ...
        if (!currentBalance || currentBalance.quantity.lessThan(returnQty)) {
          ...
          throw new AppError(
            `Insufficient stock to return "${item.name}". Available: ${available}, Return requested: ${returnQty}`,
            400
          );
        }

        const unitCost = line.unitCost !== undefined ? new Prisma.Decimal(line.unitCost) : item.costPrice;
