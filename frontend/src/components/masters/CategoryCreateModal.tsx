...
export const CategoryCreateModal: React.FC<CategoryCreateModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const createdCategory = await inventoryApi.createCategory({
        name: form.name.trim(),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-[rgba(45,45,45,0.12)] space-y-4">
        <div className="flex items-center justify-between border-b border-[rgba(45,45,45,0.08)] pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#707070]">Classify items for inventory & costing</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
          ...
          <div>
            <label className="block text-[#707070] font-semibold mb-1">Description (Optional)</label>
            <textarea
