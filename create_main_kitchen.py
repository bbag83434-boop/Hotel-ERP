...
export const MainKitchenWorkspace: React.FC<MainKitchenWorkspaceProps> = ({ initialView }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (isKitchen) {
        // Fetch kitchen config and orders
      ...
      {isKitchen ? (
        <div className="flex items-center gap-2 border-b border-[rgba(45,45,45,0.08)] pb-2 overflow-x-auto">
          {(
            ...
            <button
              key={id}
              onClick={() => setActiveKitchenTab(id)}
...
with open(r"C:\Users\Biswanath Bag\OneDrive\Desktop\Hotel-ERP\frontend\src\components\workspaces\MainKitchenWorkspace.tsx", "w", encoding="utf-8") as f:
    f.write(file_content)

print("MainKitchenWorkspace.tsx created")
